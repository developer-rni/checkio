#!/home/nikita/github/checkio/venv/bin/checkio --domain=py run non-unique-elements

# If you have 50 different plug types, appliances wouldn't be available and would be very        expensive. But once
# an electric outlet becomes standardized, many companies can design        appliances, and competition ensues,
# creating variety and better prices for consumers. -- Bill Gates
# 
...
# for solving more complex tasks.    The concept can be easily generalized for real world tasks.    For example: if
...
def checkio(data: list) -> list:
    non_unique_data_list = []
    for element in data:
        if data.count(element) != 1:
            non_unique_data_list.append(element)
    return non_unique_data_list
